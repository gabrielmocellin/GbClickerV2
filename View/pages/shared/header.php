<header>
    <div class="header_logo_button">
        <div id="hamburguer-navbar-button">
            <img class="menu-icons" src="View/pages/shared/icons/menu.png" alt="Icone abrir/fechar menu">
        </div>
        <a href='#' id="header_logo" class="noSelect">GbClicker</a>
    </div>
    <ul class="header_ul_userInfos noSelect">
        <p id="user_money_p" class="user_money noSelect"></p>
        <img draggable="false" class="user_image noSelect" src="View/pages/shared/tmp/zuckerberg_bowl.jpg">
    </ul>
</header>